...
public:
	...
	sf::RectangleShape ceillingDraw() 
	{
		sf::RectangleShape ceiling_object(sf::Vector2f(WIN_WIDTH, WIN_HEIGHT / 2));
		ceiling_object.setFillColor(sf::Color::Blue);
		return ceiling_object;
	}
	sf::RectangleShape floorDraw()
	{
		sf::RectangleShape floor_object(sf::Vector2f(WIN_WIDTH, WIN_HEIGHT / 2));
		floor_object.setPosition(0, WIN_HEIGHT / 2);
		floor_object.setFillColor(sf::Color(255, 255, 0));
		return floor_object;
	}
	void drawFov(sf::RenderWindow& window, sf::RenderWindow& window_game, maptype map)
	{
		int n = WIN_WIDTH;
		int index = 0;
		sf::RectangleShape ceiling = ceillingDraw();
		sf::RectangleShape floor = floorDraw();
		window_game.draw(ceiling);
