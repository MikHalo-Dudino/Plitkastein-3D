...
#define CEILLING_COLOR sf::Color(255, 255, 255)
#define FLOOR_COLOR sf::Color(255, 255, 255)
//player settings
...
public:
	...
	{
		sf::VertexArray ceilling(sf::Lines, WIN_HEIGHT);
		for (int i = 0; i < WIN_HEIGHT / 2; i++)
		{
			ceilling[2 * i].position = sf::Vector2f(0, i);
			ceilling[2 * i + 1].position = sf::Vector2f(WIN_WIDTH, i);
			ceilling[2 * i].color = CEILLING_COLOR;
			ceilling[2 * i].color.a = (720 - 2 * i) * 0.14;
			ceilling[2 * i + 1].color = CEILLING_COLOR;
			ceilling[2 * i + 1].color.a = (720 - 2 * i) * 0.14;			// Fuck this shit
		}
		return ceilling;
	}
	sf::VertexArray floorDraw()
	{
		sf::VertexArray floor(sf::Lines, WIN_HEIGHT);
		for (int i = 0; i < WIN_HEIGHT / 2; i++)
		{
			floor[2 * i].position = sf::Vector2f(0, WIN_HEIGHT - i);
			floor[2 * i + 1].position = sf::Vector2f(WIN_WIDTH, WIN_HEIGHT - i);
			floor[2 * i].color = FLOOR_COLOR;
			floor[2 * i].color.a = (720 - 2 * i) * 0.14;
			floor[2 * i + 1].color = FLOOR_COLOR;
			floor[2 * i + 1].color.a = (720 - 2 * i) * 0.14;			// Fuck this shit
		}
		return floor;
	}
};
